...
function compactView(viewContext) {
  ...
  if (viewContext.type === "product_list") {
    return {
      ...
      products: viewContext.products.map(compact),
    };
  }
...
function compactReplyView(viewContext) {
  ...
  const compact = (product) => ({
    ...
    price: product.price_min_usd,
    colors: product.colors,
    sizes: product.sizes,
    inStock: product.in_stock_any,
...
async function recommend(analysis) {
  return semanticSearch({
    queries: [analysis.semanticQuery, ...analysis.relatedSearchTerms],
    ...analysis.filters,
    limit: 30,
  });
}
...
async function reply(messages, viewContext, action, products, useView) {
  const conversation = splitConversation(messages);
  const replyViewContext =
    useView && ["answer", "clarify", "no_answer"].includes(action)
...
export async function chat(messages, viewContext = null) {
  ...
  const products =
    analysis.action === "recommend" ? await recommend(analysis) : null;
